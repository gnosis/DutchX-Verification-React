...
function DefaultCookies(props) {
  ...
  useDebugValue(cookieSettings.analytics ? 'analyticsOn' : 'analyticsOff')

  // ComponentDidMount
  useEffect(() => {
    const mountlogic = async () => {
      try {
        const cookieData = await localForage.getItem(props.LF_COOKIES_KEY)
        if (!cookieData) return setLoading(false)
  ...
  const handleSubmit = () => {
    ...
    return localForage.setItem(props.LF_COOKIES_KEY, { necessary, analytics })
  }

  const handleClick = e => (e.preventDefault(), setSettingsOpen(!settingsOpen))

  const { dateUpdated = 'September 2018' } = props
  const { necessary, analytics } = cookieSettings
            ...
            <div>
              ...
              <div className="disclaimerBox md-checkbox">
                <input
                  ...
                  // onChange={handleCheckbox/* () => setCookieSettings({...cookieSettings, analytics: !analytics}) */}
                  checked={analytics}
                  readOnly
                />
                <label htmlFor="disclaimer2" onClick={() => setCookieSettings({...cookieSettings, analytics: !analytics})}>Analytics</label>
              </div>
            </div>
            <button
